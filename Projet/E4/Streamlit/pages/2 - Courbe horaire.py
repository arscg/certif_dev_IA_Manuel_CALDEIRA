...
class VideoAnalytics(Analytics):
    ...
    def get_api_animov(self, sources, with_global_stats, with_detect=False):
        """
        ...
        ROUTE = f'get_data_animov_ch?sources={sources}&with_images=Single&with_detect={with_detect}&with_stats=False&with_global_stats={with_global_stats}'
        # st.write(ROUTE)
    ...
    def query_get_chevres_serie_minutes(self, source, mode='normal'):
        """
        Récupère les données de la série de chèvres par minute pour la source spécifiée.
        """
        # Vérifier si le jeton est disponible dans la session
        if 'token' not in st.session_state:
            st.error("Veuillez vous authentifier d'abord.")
        ...
        # URL de l'API pour les données par minute
        if mode == 'degrade':
            url = f'http://{END_POINT}/get_serie_last_heure'
        else:
            url = f'http://{END_POINT}/get_serie_last_heure'
    ...
    def display_graph(self, df):
        ...
        def axe_x(premier_timestamp):
            ...
            """
            timestamp_fin = premier_timestamp + timedelta(hours=1)
            debut_axe_x = premier_timestamp.strftime('%Y-%m-%d %H:%M')
            fin_axe_x = timestamp_fin.strftime('%Y-%m-%d %H:%M')
            return debut_axe_x, fin_axe_x

        tabh1, tabh2, tabh3 = st.tabs(["Courbe sur une heure", "Proportion debout/couché", "Données"])
        ...
        with tabh1:
            ...
            df_melted = df.melt(id_vars=['timestamp'], value_vars=['debout', 'couche', 'total'], 
                                var_name='État', value_name='Moyenne horaire du nombre de chèvres')
            ...
            for etat in df_melted['État'].unique():
                ...
                if etat == 'total':
                    fig.add_trace(go.Scatter(x=df_filtre['timestamp'], y=df_filtre['Moyenne horaire du nombre de chèvres'],
                                            mode=mode, name=etat,
        ...
        with tabh2:
            ...
            for etat in df_melted['État'].unique():
                ...
                fig.add_trace(go.Bar(x=df_filtre['timestamp'].dt.strftime('%Y-%m-%d %H:%M'), 
                                    y=df_filtre['Moyenne horaire du nombre de chèvres'],
                                    name=etat))

            fig.update_layout(
                title='Répartition debout/couché sur une heure.',
                xaxis_title='Date et Heure',
    ...
    def setup_radio_sources(self):
        ...
        selected_source = st.sidebar.radio("Souces", source_list, key="source_selection")
    ...
    def run(self):
        ...
        if detect:
            message = self.get_api_animov(selected_source, 'False', with_detect=True)
        else:
            message = self.get_api_animov(selected_source, 'False')

        df = self.query_get_chevres_serie_minutes(int(selected_source))

        on_line = True

        if df.empty:
            df = self.query_get_chevres_serie_minutes(int(selected_source), 'degrade')
            on_line = False
